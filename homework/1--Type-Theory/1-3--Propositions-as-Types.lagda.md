...
-- Exercise:
⇒→Type true b =  (λ p → (λ p₁ → p)) , (λ q → q tt)
⇒→Type false b = (λ p → ∅-rec ) , (λ q → tt)
```
...
not→Type true = ∅-rec , (λ q → (q tt))
not→Type false = (λ p → (λ p₁ → ∅-rec p₁)), (λ q → tt)
```
...
or→Type-fro false b (inl p) = ∅-rec p 
...
≡ℕ-sym (suc n) zero p = p
≡ℕ-sym zero (suc m) p = p
...
≤ℕ-trans zero (suc m) (suc k) p q = ≤ℕ-trans zero m k p q
≤ℕ-trans (suc n) (suc m) (suc k) p q = ≤ℕ-trans n m k p q
